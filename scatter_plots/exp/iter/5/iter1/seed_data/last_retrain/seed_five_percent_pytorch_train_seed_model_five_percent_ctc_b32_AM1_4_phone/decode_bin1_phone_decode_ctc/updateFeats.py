...
def get_parser():
    ...
    parser.add('--recog_file_name', type=str)
...
def createBinJson(feat_dir_name, dir_name, feat_file_name="data",
        train_file_name="", recog_file_name=""):
    ...
    #writeNewFeats(seed_b1, dir_name, 'bin1P_and_seed_on_phone')
    #writeNewFeats(b, dir_name, 'bin1P_on_phone')
    writeNewFeats(seed_n_bin, dir_name, train_file_name)
    writeNewFeats(b, dir_name, recog_file_name)

def main(args):
    parser = get_parser()
    args = parser.parse_args(args)
    createBinJson(args.feat_dir_name, args.out_dir_name, args.feat_file_name,
            args.train_file_name, args.recog_file_name)
